  ...
  return <>
    <section className="hero">
      ...
      <div className="container hero-content">
        ...
        <p>Serving all Lebanon since {companyInfo.established} with professional elevator installation, repair, maintenance, modernization, and safety inspection services.</p>
    ...
    <section className="section">
      <div className="container about-preview">
        ...
        <div>
          ...
          <h2>WE LIFT STANDARDS.<br />WE BUILD TRUST.</h2>
          <p>Founded in {companyInfo.established}, Gentle Elevators provides safe, reliable elevator solutions from our base in Saida to customers throughout Lebanon.</p>
          <div className="trust-grid">
            {trust.map(({ icon: Icon, title, text }) => <div className="trust" key={title}><Icon /><h3>{title}</h3><p>{text}</p></div>)}
          </div>
          <Link className="text-link" href="/about">Discover our company <ArrowRight /></Link>
        </div>
      </div>
    </section>
    ...
    <section className="section services-preview">
      <div className="container">
        <div className="center-heading">
          ...
          <p>We provide complete elevator services based on your building&apos;s needs.</p>
        </div>
        <div className="service-grid">
          {services.slice(0, 4).map(({ icon: Icon, ...service }) => <article className="service-card" key={service.slug}>
            ...
            <div><Icon /><h3>{service.title}</h3><p>{service.description}</p><Link href={`/services#${service.slug}`}>Learn more <ArrowRight /></Link></div>
          </article>)}
        </div>
      </div>
    </section>
    ...
    <section className="section why">
      <div className="container">
        <Eyebrow>Why Gentle</Eyebrow>
        <h2>WHY CHOOSE GENTLE ELEVATORS?</h2>
        <div className="why-grid">{why.map(item => <div key={item}><Check /><span>{item}</span></div>)}</div>
      </div>
    </section>
